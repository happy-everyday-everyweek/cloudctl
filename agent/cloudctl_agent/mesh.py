...
1. 只用标准库（socket / http.server / urllib / threading），不引入任何 UI 或
   图形依赖。设备端只当被控端，不弹窗、不托盘、不提示。
2. 发现用 UDP：收组播 239.255.42.99，发送同时走组播与广播，兼容禁组播的交换机。
3. 通信走局域网 HTTP，每个请求带共享令牌；对端命令进入同一套 Router 权限门禁。
4. 支持中继：目标不在直连邻居里时，交给已知邻居转发，ttl / hops 上限防止风暴。

协议版本 1，信封字段：v、kind、mesh、from、from_name、to、id、ts、hops。
"""
...
class MeshService:
    ...
    def __init__(self, cfg, log, run_cmd: Callable[..., dict] | None = None, agent_version: str = "1.1.0") -> None:
        self.cfg = cfg
        self.log = log
        self.run_cmd = run_cmd or (lambda op, args, timeout=60.0: {"ok": False, "err": "命令执行器未就绪"})
        self.version = agent_version
        self.mesh = cfg.mesh_group or cfg.group or "default"
        self.token = cfg.mesh_token or cfg.devsrv_secret
    ...
    def start(self) -> dict:
        ...
        self.log.info("局域网互联已启动 mesh=%s http=%s:%s 发现端口=%s",
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "rejected": self.rejected, "accept_cmd": bool(self.cfg.mesh_accept_cmd),
    ...
    def _send_announce(self) -> None:
        ...
        targets = [self.cfg.mesh_mcast, BROADCAST]
        for host in targets:
            try:
    ...
    def _prune(self) -> None:
        ...
        with self._lock:
            for did in [k for k, v in self.peers.items() if v.get("last_seen", 0) < limit]:
    ...
    def _exec(self, env: dict) -> dict:
        if not self.cfg.mesh_accept_cmd:
            self.rejected += 1
            return {"ok": False, "err": "本机不接受对端命令（mesh_accept_cmd=false）"}
        if self._dedup(env):
        ...
        ttl = max(1, int(self.cfg.mesh_max_hops))
        _ = ttl
    ...
    def do_POST(self) -> None:
        ...
        if body.get("from"):
            self.ms._register({"from": body["from"], "from_name": body.get("from_name"),
                               "group": body.get("group"), "port": body.get("port") or self.ms.cfg.mesh_port,
                               "ver": body.get("ver")}, self._peer_ip())
